...
**Bacalhau / beijinhos / lunch (EP61–63):** `dona_bacalhau` (giant salt cod) · `dona_kiss-phone` (blowing a kiss on the phone) · `otto-robe_shock` · `otto-robe_brush` (bathrobe, towel turban) · `boss_phone` (British boss, desk) · `bank_headset` · `otto-desk_sandwich` · `otto-desk_asleep` · `colleagues_shock` (pointing left) · `colleagues_lunch` (waist-up, coffee and wine)
**Directions / August / clubs (EP64–66):** `otto-casual_map` · `lady_point` (points right) · `oldmen_argue` · `oldmen_escort` (arm in arm with Otto, walking right) · `otto-summer_toothache` · `beach_workers` (top-down: baker, barber, dentist on towels) · `fan_red` · `fan_green` · `fan_blue` (plain striped shirts and scarves, no logos) · `otto-casual_scarves` · `otto-casual_shrug` · `marta_office`
**Vegetarian / heist / voice note (EP67–69):** `dona_pig` (carrying a roast pig) · `buck-chair_aww` · `mum_record` · `mum_shout` · `mum_baguette` (Portuguese mum, apron) · `otto-casual_baguette` · props `roast-pig` · `pigeons_roof` (4 in sunglasses with a nata) · `pigeon_brains` · `pigeon_muscle` · `pigeon_distraction` · `pigeon_lookout`
**Remote job / beach family / EDP (EP70–72):** `priest_bless` · `gossip_ladies` · `barman_nervous` (mint-cut) · `buck_towel` · `buck_sardine` · `beach_family` (walking left, loaded) · `dona_beach` (swimsuit, beach chair) · `teen_phone` · `teen_blanket` · `ze_angry` · `ze_frame` (frame: x .35–.68, y .31–.48) · props `gear_parasol` · `gear_chairs` · `gear_cooler` · `gear_pot` · `gear_grill` · `gear_melon` · `gear_tv` · `gear_canary`
