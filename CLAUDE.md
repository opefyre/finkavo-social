...
When the owner names a tool (`reel`, `carousel`, `topics`, `queue`, `publish`, `pipeline`, `spare`), open **TOOLS.md** and
...
- **Drafts only.** Create posts in Buffer as drafts; the owner schedules them. Never publish.
...
- **The pipeline is off.** Do not start, enable or edit it unless asked.
