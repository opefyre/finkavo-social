...
**Private operations board:** `https://approve.finkavo.com/board` is the owner-only, read-only operational view. Cloudflare Access gates the hostname, and the API independently verifies the signed Access JWT, exact application audience, issuer, expiry, and allowed owner email. The Kanban groups every retained post into Drafts, Review, Approved, Scheduled, Published, or Attention. Search and filters cover topic/copy, category, risk, and plan date. Post details show exact rendered media, final caption, sources, approval and publishing timestamps, provider/error state, identifiers, and the latest activity events. Responses are non-cacheable and protected by a nonce-based content-security policy; the board exposes no mutation controls or API credential.
