...
**Daily batch recovery rule:** At 08:02 Lisbon, generation must continue until five valid same-day posts exist. Each topic receives an initial generation plus two targeted repairs. A persistent failure marks that concept blocked, replaces the slot with a current verified non-duplicate reserve brief, and repeats for at most six replacement rounds. A successful batch immediately sends every new draft to Discord; the 08:10–08:55 review workflow is only an idempotent safety net. The daily report and incomplete-batch alert run at 09:00, after recovery, and identify generation failures separately from source verification.

**Private operations board:** `https://approve.finkavo.com/board` is the owner-only operational view. Cloudflare Access gates the hostname, and the API independently verifies the signed Access JWT, exact application audience, issuer, expiry, and allowed owner email. The Kanban groups every retained post into Drafts, Review, Approved, Scheduled, Published, Attention, or Archived. Search and filters cover topic/copy, category, risk, and plan date. Post details show exact rendered media, final caption, sources, approval and publishing timestamps, provider/error state, identifiers, and the latest activity events. Review items can be approved or rejected from the board with revision-bound decisions. Attention items can reopen a rejection, edit copy into a new immutable draft revision, retry the exact approved render, retry a confirmed publish failure, explicitly retry an ambiguous Buffer handoff only after the owner confirms no matching Buffer post exists, or soft-archive. Drafts can be quality-checked and sent to Review and Discord from the board. Every action requires owner authentication, a short-lived post/revision/reviewer-bound HMAC token, explicit confirmation, and an append-only audit event. Responses are non-cacheable and the board exposes no API credential.
...
Explicit Buffer/Instagram delivery failures identify the topic, post and job IDs, failed Buffer post, intended time, provider state, recovery action, and exact board card. The immutable post returns to the durable local queue and is assigned to the next free slot without regeneration or reapproval.
...
- Daily generation is item-isolated: a quality failure for one planned topic is recorded and skipped, while other verified topics continue to generation and Discord review. An empty review queue is a normal no-op, not a workflow failure.
