...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if(req.method==="POST"&&url.pathname==="/v1/reserve/replace-held"){
      ...
      for(const slot of heldSlots){const samePillar=eligible.filter(card=>card.subjectFamily===slot.pillar);const pool=samePillar.length?samePillar:eligible;if(!pool.length)break;const card=pool[Number(createHash('sha256').update(`${day}:${slot.slot_number}`).digest().readUInt32BE(0))%pool.length];eligible=eligible.filter(item=>item.id!==card.id);const source=evidence.find(row=>row.canonicalUrl===card.sourcePolicy.canonicalUrl)!;const sources=[{documentId:String(source.document_id),url:card.sourcePolicy.canonicalUrl,title:String(source.title),publisher:String(source.authority),tier:'official',locale:'pt',retrievedAt:String(source.verifiedAt),contentHash:String(source.content_hash),relevanceScore:100,matchedTerms:card.evidenceTerms,excerpts:evidenceWindows(String(source.visible_text),card.evidenceTerms)}];const bundleHash=hash(sources);const [bundle]=await sql`INSERT INTO social_topic_evidence_bundle(plan_slot_id,bundle_hash,sources,verification_state,verified_at,expires_at) VALUES(${slot.id},${bundleHash},${sql.json(sources)},'verified',now(),now()+(${card.sourcePolicy.freshnessDays}::STRING||' days')::INTERVAL) ON CONFLICT(plan_slot_id,bundle_hash) DO UPDATE SET verification_state='verified',verified_at=now(),expires_at=excluded.expires_at RETURNING id`;const fingerprint=`held-fallback:${slot.plan_version}:${day}:${slot.slot_number}:${card.id}`;const [concept]=await sql`INSERT INTO social_post_concept(document_id,topic,category,risk_level,priority,timeliness,fingerprint,status,planned_for,reason,repeat_allowed,score,plan_slot_id,evidence_bundle_id,subject_family,user_question,content_intent) VALUES(${source.document_id},${card.topic},${card.subjectFamily},'medium',${100-Number(slot.slot_number)},'evergreen',${fingerprint},'planned',${day},${`Evidence hold replacement for: ${slot.topic}`},false,${100-Number(slot.slot_number)},${slot.id},${bundle.id},${card.subjectFamily},${card.userQuestion},${card.contentIntent}) ON CONFLICT(fingerprint) DO UPDATE SET evidence_bundle_id=excluded.evidence_bundle_id,status=CASE WHEN social_post_concept.status='used' THEN 'used' ELSE 'planned' END,updated_at=now() RETURNING id`;await sql`UPDATE social_editorial_plan_slot SET topic=${card.topic},pillar=${card.subjectFamily},audience=${card.audience},timing_class='evergreen',reserve_kind='evidence_hold_fallback',search_terms=${sql.json(card.evidenceTerms)},required_authority=${card.sourcePolicy.canonicalUrl},subject_family=${card.subjectFamily},user_question=${card.userQuestion},content_intent=${card.contentIntent},occurrence_key=NULL,campaign_stage='evidence_hold_fallback',brief=${sql.json(card)},status='evidence_ready',updated_at=now() WHERE id=${slot.id}`;await sql`INSERT INTO social_event(event_type,payload) VALUES('planning.held_replaced',${sql.json({slotId:slot.id,originalTopic:slot.topic,reserveId:card.id,replacementTopic:card.topic,conceptId:concept.id})})`;replacements.push({slotId:slot.id,originalTopic:slot.topic,reserveId:card.id,topic:card.topic});}
