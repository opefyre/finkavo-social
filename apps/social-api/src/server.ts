  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/v1/evidence/research") {
      ...
      for (const slot of slots) {
        ...
        // The slot then holds evidence nobody can generate from, recovery finds no work
        // and reports none, and the day quietly ends one post short. Two days running,
        // each day's second slot was bound to a concept spent a week earlier.
        //
        // A spent concept means this slot has no work, which is exactly what `held` means
        // and what the evergreen reserve exists to answer. Handing it over is what a
        // person had to do by hand both mornings.
        const conceptIsSpent = String(concept.status) !== 'planned';
        if (conceptIsSpent) {
          await sql`UPDATE social_editorial_plan_slot SET status='held',updated_at=now() WHERE id=${slot.id}`;
          await sql`INSERT INTO social_event (event_type,payload) VALUES ('planning.slot_held_for_spent_concept',${sql.json({ slotId: slot.id, conceptId: concept.id, conceptStatus: String(concept.status), topic: String(slot.topic) })})`;
          results.push({ slotId: slot.id, topic: slot.topic, state: 'held', reason: `its concept is already ${String(concept.status)}; the reserve should replace this slot` });
          continue;
        }
    ...
    if(req.method==="POST"&&url.pathname==="/v1/reserve/replace-held"){
      ...
      // A card holds exactly one concept row, so it can be attached to exactly one
      // slot. Eligibility was deduplicated only within a single request, so the same
      // card was handed to held slots on different days; the row followed the last
      // write and the earlier slot was left marked evidence_ready with nothing behind
      // it. A card already reserved for a pending slot is not available again until
      // that slot has been turned into a post.
      const reservedRows=await sql`SELECT fingerprint FROM social_post_concept WHERE status='planned' AND plan_slot_id IS NOT NULL AND fingerprint LIKE 'held-fallback:%'`;
      const reserved=new Set(reservedRows.map(row=>String(row.fingerprint)));
      const reserveFingerprint=(card:{id:string;topic:string;userQuestion:string;audience:string;contentIntent:string})=>`held-fallback:${hash({id:card.id,topic:card.topic,userQuestion:card.userQuestion,audience:card.audience,contentIntent:card.contentIntent}).slice(0,16)}`;
      eligible=eligible.filter(card=>!reserved.has(reserveFingerprint(card)));
      const replacements=[];
