...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if(req.method==="POST"&&url.pathname==="/v1/news/decide-flex-slot"){
      ...
      const news=newsRows.find(row=>(row.excerpts as string[]|null)?.length&&!findDuplicate({topic:row.topic,subject_family:row.category,user_question:row.topic,content_intent:'timely_news',postIntent:'timely_news'},existing as DuplicateCandidate[]));
